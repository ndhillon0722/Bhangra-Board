(() => {
  ...
  const renderVariants = (clip, review) => {
    ...
    clip.variants.forEach((variant, index) => {
      ...
      audio.src = `/static/review-audio/${variant.path
        .split("/")
  ...
  const save = async (moveNext) => {
    try {
      ...
      const response = await fetch(
        `/audio-review/api/reviews/${encodeURIComponent(clip.id)}`,
        {
          method: "PUT",
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (/^[0-9]$/.test(event.key)) {
      const variantIndex = event.key === "0" ? 9 : Number(event.key) - 1;
      const audio = elements.variants.querySelectorAll("audio")[variantIndex];
      if (audio) {
  ...
  fetch("/audio-review/api/queue")
    .then((response) => response.json())
    .then((payload) => {
      state.queue = payload.queue;
      state.reviews = payload.reviews;
      const firstUnreviewed = state.queue.clips.findIndex(
        (clip) => !state.reviews[clip.id]
      );
      state.index = firstUnreviewed === -1 ? 0 : firstUnreviewed;
      render();
    })
    .catch((error) => {
      elements.empty.hidden = false;
